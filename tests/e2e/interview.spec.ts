...
test("Interview two sum", async ({ page }) => {
  ...
  await expect(
    page.locator(".flex.gap-3.flex-row-reverse > .rounded-lg")
  ).toBeVisible();
  await expect(
    page.locator("div:nth-child(3) > .rounded-lg").first()
  ).toBeVisible();

  await page.getByRole("button", { name: "Run Testcases" }).click();
  await expect(
    page.locator(".flex.flex-col.gap-4 > div:nth-child(4) > .rounded-lg")
  ).toBeVisible();
  await page.locator("div:nth-child(5) > .rounded-lg").click();
